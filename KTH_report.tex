\documentclass[11pt,draft]{report} % Dokumentenklasse
...
Basically my main task to be completed during the internship, was the development of an algorithm to postprocess the data obtained by the measurement in a special way. 
...
In general, acoustic liners are components to damp {\color{red} incoming} sound waves using the principle of Helmholtz resonance to dissipate energy.
\fxnote{The above sentence is not completely correct. A Helmholtz resonator does not dissipate energy as it is a reactive element, however with flow acoustic energy is converted into flow energy. Furthermore, the optimum working of the liner is not based on the resonance frequency of the cavity, but on the so-called Cremer's impedance. When the liner has this impedance the damping is optimal, but it is not at the resonance frequency of the cavity.}
...
{\color{red} NOT CORRECT Therefore, a model that simulates the sound spectrum of a fan in a duct flow is set up.}
\fxnote{The impedance is determined for a range of frequencies, which are not based on the sound spectrum of the fan, it should be of course have the same span as the sound spectrum of the fan}
...
The governing relation between the measured pressures and how to determine the impedance is going to be presented more detailed later on. 

Graph Experimental setup
	

\subsection{The wave equation and its solutions}
\fxnote{You should use the convected helmholtz equation, because in the current equations you don't take into account the effects of flow. }
\fxnote{Give references to the literature where you have taken the equations from}
\fxnote{You should introduce all the used variables when presenting an equation, for example what is $p$ and $t$ in the wave equation, what is $\omega$ etc.}
...
\begin{equation}
    \nabla^2p-\frac{1}{c^2}\frac{\partial^2p}{\partial^2t} {\color{red} = 0}
\end{equation}. 
...
\fxnote{Not correct, at one side of the wall $y=0$ the acoustic particle velocity is zero, but on the other side, $y=b$ the acoustic impedance is given, that is the ratio between the pressure and the acoustic particle velocity. See also the theory document} 
...
\fxnote{Not correct when the right boundary conditions are been taken into account}
...
\fxnote{Not correct when the right boundary conditions are taken into account, the formula only holds for a rectangular duct with all the walls rigid).
...
\fxnote{This equation is the correct one when describing the wave numbers ($k_{y,n}$ in equation 2.8). When $z_{ac} \rightarrow \infty$ than this equation reduces to 2.8}
...
\fxnote{You can move this part to the third chapter. The Prony method is not specifically related to the acoustic domain, it can also be used for different problems}
...
\fxnote{Rewrite the above part, it is a bit messy}
...
\section{Basic ideas}
\fxnote{This part is very unclear, we use the Prony method to get the wavenumbers of the modes that are present in the duct. However, this doesn't have to be in the basic idea as it is not related to the idea of the Prony method.}
...
\fxnote{This part is unclear as well, what do the equations mean, are they the same? What is $N$ $s_j$?}
