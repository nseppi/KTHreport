...
\end{figure}
\fxnote{Give a reference to the figure in the text somewhere}
...
\fxnote{You didn't include the new figures in the commit, so I don't have them, but double check if your choice of reactangular basis is shown somewhere, so that it is clear what the $x$, $y$ and $z$ direction are.}
...
\fxnote{This is only true for the hard wall cased, that is for the x-direction. The y-direction has the solution that is shown in the next equation}
\begin{subequations}
\begin{align}
k_{x,m}&=\frac{m\pi}{W}\\
k_{y,n}&=\frac{n\pi}{H}
...
\end{equation}
\fxnote{You should refer to the above equation}
...
Z(z)\propto e^{-ik_z^j}
\end{equation} 
\fxnote{Explain the superscript $j$, or don't use it in this equation}
...
\fxnote{Is it true for this Shannon Nyquist theorem? Or does it just give rise to aliasing which can be fixed?}
...
\fxnote{Can you state the requirements on b and L a bit more mathematically? Do they have to be real numbers, integers etc.}
...
\fxnote{Is this really true? A non-truncated gaussian in the space domain gives a non-truncated gaussian in the fourier transformed domain. I have a feeling that this is not true for a truncated gaussian. Do you have a reference?}
